...
-- ==========================================================
-- 🚀 AUTOMAÇÃO 1: STORED PROCEDURE
-- ==========================================================

-- Stored Procedure para listar as consultas de um paciente pelo CPF
-- Cria o procedimento para executar uma busca complexa
CREATE PROCEDURE SP_ListarConsultasPaciente (@cpf_paciente varchar(14))
AS
BEGIN
    SELECT 
        c.numero_consulta AS 'Número da Consulta',
        c.data_consulta AS 'Data',
        c.horario_consulta AS 'Horário',
        m.nome_medico AS 'Médico',
        m.especialidade AS 'Especialidade'
    FROM 
        consulta c
    JOIN 
        paciente p ON c.fk_paciente_cpf = p.cpf
    JOIN 
        medico m ON c.fk_medico_crm = m.crm
    WHERE 
        p.cpf = @cpf_paciente;
END
GO
...
-- Lista as consultas da paciente Bruna Alvez
EXEC SP_ListarConsultasPaciente '123.456.789-12';
...
-- Cria uma View (Tabela Virtual) para listar apenas os pacientes com plano 'Inovamed'
CREATE VIEW vw_pacientes_inovamed AS
SELECT
    cpf,
    nome_paciente,
    telefone,
...
-- 🧪 TESTANDO A VIEW
-- Consulta a View de forma simplificada
SELECT * FROM vw_pacientes_inovamed;